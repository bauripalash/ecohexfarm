#ifndef PB_HEXBUG_H
#define PB_HEXBUG_H

#include "raylib.h"
#include <stdbool.h>

#define DEFAULT_BUG_SIZE    6
#define DEFAULT_BUG_THICK   1

#define BUG_MAX_HEALTH      255
#define BUG_MIN_HEALTH      1

#define BUG_MIN_RANGE       1
#define BUG_MAX_RANGE       255

#define BUG_MIN_RANGE_VALUE DEFAULT_BUG_SIZE
#define BUG_MAX_RANGE_VALUE DEFAULT_BUG_SIZE * 20

#define BUG_MAX_SPEED       100
#define BUG_MIN_SPEED       1

#define BUG_BASE_RANGE      DEFAULT_BUG_SIZE * 3
#define BUG_BASE_HEALTH     100
#define BUG_BASE_SPEED      100

typedef struct HexGene {
    // Health
    int red;
    // Speed
    int green;
    // Range
    int blue;
    // hex color
    int hGene;
    Color hColor;
} HexGene;

typedef struct HexBug {
    int id;
    int tile;
    int nextTile;
    int target;
    int size;
    Vector2 pos;
    HexGene gene;
    bool moving;
} HexBug;

HexBug NewGenesisBug(bool primary, int tile);
HexBug NewHexBug(int color);
void DrawHexBug(HexBug *bug);
void BugWalkToTarget(HexBug *bug);
#endif
